...
7. **Given** some metrics fail during an aggregate `/zte/lte/get`, **When** the daemon builds the response, **Then** it publishes only the successful metrics to `/zte/lte` and logs which metrics failed.
