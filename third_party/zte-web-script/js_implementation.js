...
window.cookies = window.cookies || {
  ...
  delete: function (name) {
    document.cookie =
      "{0}=; Path=/; Expires=Thu, 01 Jan 1970 00:00:01 GMT;".replace(
        "{0}",
        name,
      );
  },
...
function wait_for_log_in() {
  check_log_in(
    function () {
      ...
      show_logout_and_shutdown_buttons_timer_id = window.setInterval(
        function () {
...
function make_hidden_settings_visible() {
  alert(
    "This option makes hidden device settings visible.\n" +
        ...
        success: function (a) {
          ...
          if ("0" == j.result) {
            if (save_password_hash)
              cookies.set("admin_password_hash", password_hash);
            ...
            switch (j.result) {
              ...
              case "3": {
                ...
                if (have_admin_password_hash()) {
                  console.log(
                    "Wrong password. Removing stored password hash ...",
                  );
            ...
            alert(
              (developer_login ? "Developer login" : "Login") +
                " failed! Reason: " +
                reason +
                ".",
            );
...
function enable_automatic_login() {
  var res = confirm(
    "You can make this script log in for you\n" +
  ...
  $.ajax({
    ...
    success: function (a) {
      if (a.loginfo.toLowerCase() == "ok") {
        if (logged_in_callback) logged_in_callback();
      } else {
        if (not_logged_in_callback) not_logged_in_callback();
      }
    },
    error: err,
  });
...
function parse_lte_cell_info() {
  ...
  var lte_main_band = lte_ca_pcell_band != "" ? lte_ca_pcell_band : lte_band;

  if (lte_main_band == "") lte_main_band = "??";
  ...
  lte_cells.push(
    new LteCaCellInfo(
      ...
      (lte_ca_pcell_bandwidth != "" ? lte_ca_pcell_bandwidth : bandwidth)
        .replace("MHz", "")
        .replace(".0", ""),
      lte_rssi,
      lte_rsrp_1,
  ...
  for (var i = 0; i < scell_infos.length; i++) {
    if (scell_infos[i] == "") continue;

    var scell_info = scell_infos[i].split(",");
    var have_scell_sig_info = scell_sig_infos.length > i;
    var scell_sig_info = have_scell_sig_info
      ? scell_sig_infos[i].split(",")
      : undefined;

    if (scell_info.length < 6) continue;

    if (have_scell_sig_info && scell_sig_info.length < 3) continue;
    ...
    lte_cells.push(
      new LteCaCellInfo(
        ...
        "", // RSSI
        (have_scell_sig_info ? scell_sig_info[0] : "").replace(
          "-44.0",
          "?????",
        ), // RSRP
        "",
...
function parse_nr_cell_info() {
  ...
  if (_5g_rx0_rsrp == "") _5g_rx0_rsrp = Z5g_rsrp;

  var nr_cells = [];

  var allowed_nr_bands = (
    is_5g_nsa ? nr5g_nsa_band_lock : nr5g_sa_band_lock
  ).split(",");

  if (!is_ca) {
    var nr_band = is_5g_nsa ? "n" + nr5g_action_nsa_band : nr5g_action_band;
  ...
  nr_multi_ca_scell_info.split(";").forEach((cell) => {
     ...
     */
    if (allowed_nr_bands.indexOf(nr_band) == -1) return;
  ...
  if (
    false &&
    typeof previous_nr_cells !== "undefined" &&
    nr_cells.length == previous_nr_cells.length
  ) {
    for (var i = 1; i < nr_cells.length; i++) {
      if (
        nr_cells[i].rsrp1 == previous_nr_cells[i].rsrp1 &&
        nr_cells[i].sinr == previous_nr_cells[i].sinr
      ) {
        nr_cells[i].unchanged_updates =
          previous_nr_cells[i].unchanged_updates + 1;
  ...
  $.ajax({
    ...
    success: function (a) {
      ...
      is_umts =
        ...
        network_type == "UMTS" ||
        network_type == "CDMA" ||
        network_type == "CDMA_EVDO" ||
        network_type == "EVDO_EHRPD" ||
      ...
      is_lte_plus =
        wan_lte_ca &&
        (wan_lte_ca == "ca_activated" || wan_lte_ca == "ca_deactivated");
      ...
      if (
        tx_power != "" &&
        is_lte &&
        !is_5g_nsa /* Prevent showing an outdated value from an LTE session */
      ) {
        tx_power +=
          " dBm (" + Math.pow(10, tx_power / 10.0).toFixed(3) + " mW)";
        $("#txp").show();
      } else $("#txp").hide();

      $("#ca_active").html(
        wan_lte_ca && wan_lte_ca == "ca_activated" ? "&#10003;" : "&#10005;",
      );
      ...
      if (
        ...
        var temp = "";
        if (pm_sensor_ambient && pm_sensor_ambient > -40)
          temp +=
            (temp ? "&nbsp;&nbsp;" : "") +
            "A:&nbsp;" +
            pm_sensor_ambient +
            "°c";
        if (pm_sensor_mdm && pm_sensor_mdm > -40)
          temp +=
            (temp ? "&nbsp;&nbsp;" : "") + "M:&nbsp;" + pm_sensor_mdm + "°c";
        if (pm_sensor_5g && pm_sensor_5g > -40)
          temp +=
            (temp ? "&nbsp;&nbsp;" : "") + "5G:&nbsp;" + pm_sensor_mdm + "°c";
        if (pm_sensor_pa1 && pm_sensor_pa1 > -40)
          temp +=
            (temp ? "&nbsp;&nbsp;" : "") + "P:&nbsp;" + pm_sensor_pa1 + "°c";
        if (wifi_chip_temp && wifi_chip_temp > -40)
          temp +=
            (temp ? "&nbsp;&nbsp;" : "") + "W:&nbsp;" + wifi_chip_temp + "°c";
        $("#temps").html(temp);
        $("#temperature").show();
...
function lte_cell_lock(reset = false) {
  ...
  if (reset) {
    ...
    var cellLockDetails = prompt(
      "Please input PCI,EARFCN, separated by ',' char (example 116,3350). " +
        "Leave default for lock on current main band.",
      defaultPciEarfcn,
    );
    ...
    var pciIsValid =
      !isNaN(inputValues[0]) && Number.isInteger(parseFloat(inputValues[0]));
    var earfcnIsValid =
      !isNaN(inputValues[1]) && Number.isInteger(parseFloat(inputValues[1]));
          ...
          if (response.result === "success") {
            var rebootMessage =
              ...
              (reset
                ...
                : "for the cell lock to be active") +
              ".\n\nReboot now?";
  ...
  if (reset) {
    ...
    if (nrCellInfo.length > 0) {
      ...
      defaultCellDetails =
        ...
        primaryNrCell.arfcn +
        "," +
        primaryNrCell.band.replace("n", "") +
        "," +
        "30";
    }

    cellLockDetails = prompt(
      "Please input PCI,ARFCN,BAND,SCS separated by ',' char (example 202,639936,78,30). " +
        "Leave default for locking the current NR primary band. You may need to adjust the SCS.",
      defaultCellDetails,
    );
    ...
    if (cellLockDetails === null || cellLockDetails.trim() === "") {
      ...
      var arfcnIsValid =
        !isNaN(inputValues[1]) && Number.isInteger(parseFloat(inputValues[1]));
      var bandIsValid =
        !isNaN(inputValues[2]) && Number.isInteger(parseFloat(inputValues[2]));
      var scsIsValid = ["15", "30", "60", "120", "240"].includes(
        inputValues[3],
      );

      if (!pciIsValid || !arfcnIsValid || !bandIsValid || !scsIsValid) {
        alert(
          "Invalid input. Please ensure all values are correctly formatted.",
        );
        return;
      }
    }
              ...
              if ("success" == j.result) {
                if (nested_attempt_with_dev_login) {
                  if (!had_admin_password_hash)
                    alert(
                      "Successfully performed LTE band lock with developer login ...",
                    );
                }
              } else {
                if (!nested_attempt_with_dev_login && !logged_in_as_developer) {
                  ...
                  alert(
                    "LTE band locking with developer login still failed.\nThere might be something else wrong.",
                  );
                }
              }
            },
...
function bridge_mode(enable) {
  if (
    !confirm(
      (enable ? "Enable" : "Disable") + " bridge mode and reboot router?",
    )
  )
    return;
...
function arp_proxy(enable) {
  if (
    !confirm((enable ? "Enable" : "Disable") + " ARP proxy and reboot router?")
  )
    return;
...
function reboot(force = false) {
  if (!force && !confirm("Reboot Router?")) return;
...
function set_config(key, value) {
  if (
    typeof require.s.contexts._.defined["config/config"][key] === "undefined"
  ) {
    console.log("Config key not found, setting it anyway: " + key);
  } else {
    console.log(
      "Previous config value: " +
        require.s.contexts._.defined["config/config"][key],
    );
  }
